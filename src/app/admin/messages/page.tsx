...
const messagesData: Message[] = [
  {
    ...
    content: 'I have not received payment for my order delivered last week. Please help resolve this issue.',
    sender: {
      name: 'John Smith',
  ...
  },
  {
    ...
    content: 'Could you please clarify the quality standards required for organic vegetables?',
    sender: {
      name: 'Sarah Johnson',
  ...
  },
  {
    ...
    content: 'I wanted to share my positive experience with AgriLink. The platform has helped me connect with reliable suppliers.',
    sender: {
      name: 'Michael Chen',
  ...
  },
  {
    ...
    content: 'I am having trouble scheduling deliveries through the platform. The calendar feature seems to be not working properly.',
    sender: {
  ...
  },
  {
    ...
    content: 'I need help with verifying my account. I have submitted all required documents but still waiting for approval.',
    sender: {
      name: 'David Thompson',
              ...
              {currentMessages.map((message) => (
                <tr key={message.id} className={`hover:bg-gray-50 ${!message.read ? 'bg-blue-50' : ''}`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getRoleColor(message.sender.role)}`}>{message.sender.role}</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getCategoryColor(message.category)}`}>{message.category}</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getPriorityColor(message.priority)}`}>{message.priority}</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getPriorityColor(selectedMessage.priority)}`}>{selectedMessage.priority} priority</span>
                    <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getCategoryColor(selectedMessage.category)}`}>{selectedMessage.category}</span>
                    <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(selectedMessage.status)}`}>{selectedMessage.status}</span>
                  </div>
                </div>
                <button
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getRoleColor(selectedMessage.sender.role)}`}>{selectedMessage.sender.role}</span>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-gray-50 p-3 rounded-lg">
                    ...
                    <div className="font-medium text-gray-900">{selectedMessage.id}</div>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <div className="text-sm text-gray-600">Received</div>
                    <div className="font-medium text-gray-900">{formatTimestamp(selectedMessage.timestamp)}</div>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <div className="text-sm text-gray-600">Category</div>
                    <div className="font-medium text-gray-900 capitalize">{selectedMessage.category}</div>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <div className="text-sm text-gray-600">Priority</div>
                    <div className="font-medium text-gray-900 capitalize">{selectedMessage.priority}</div>
                  </div>
                </div>
              </div>
              {/* Read Status */}
                ...
                <div className="flex items-center gap-2">
                  {selectedMessage.read ? (
                    <>
                      ...
                      <span className="text-sm text-green-600">Message marked as read</span>
                    </>
                  ) : (
                    <>
                      <FiMail className="w-4 h-4 text-orange-600" />
                      <span className="text-sm text-orange-600">Message was unread</span>
                    </>
                  )}
                </div>
              </div>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-opacity-30 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-sm w-full p-6">
            ...
            <p className="mb-6 text-gray-700">Are you sure you want to delete this message?</p>
            <div className="flex justify-end gap-3">
              <button
