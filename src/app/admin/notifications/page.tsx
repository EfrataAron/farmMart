...
const notificationsData = [
  ...
  },
  {
    ...
    priority: 'critical',
    title: 'System Alert',
    message: 'Database connection is running slow. Performance may be affected.',
...
export default function NotificationsPage() {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'user': return <FiUsers className="w-5 h-5" />
      case 'system': return <FiSettings className="w-5 h-5" />
      case 'order': return <FiShoppingCart className="w-5 h-5" />
      case 'product': return <FiPackage className="w-5 h-5" />
      case 'security': return <FiShield className="w-5 h-5" />
      case 'message': return <FiMessageSquare className="w-5 h-5" />
      case 'revenue': return <FiTrendingUp className="w-5 h-5" />
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'revenue': return 'text-indigo-600 bg-indigo-50'
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${getPriorityColor(notification.priority)}`}>
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 ml-4">
                      ...
                      >
                        {notification.read ? <FiMail className="w-4 h-4" /> : <FiCheckCircle className="w-4 h-4" />}
                      </button>
                      <button
